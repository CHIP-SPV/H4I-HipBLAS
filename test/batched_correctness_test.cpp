...
// Written to mirror test/two_hip.cpp exactly: raw mallocs (no std::vector), the same
// intermediate LU+ipiv print, the same warm-up loop, and a direct inline tolerance
// check (no compareArrays).
bool testDgetrsKnownSolutionTranspose() {
    ...
    bool ok = (std::abs(X[0] - 1.0) <= TOLERANCE_DOUBLE) &&
              (std::abs(X[1] - 2.0) <= TOLERANCE_DOUBLE);
